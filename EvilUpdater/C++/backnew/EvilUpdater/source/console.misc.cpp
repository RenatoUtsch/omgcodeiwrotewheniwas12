...
/// EvilUpdater - EvilUpdater/EvilUpdater/source/console.misc.cpp

// Misc functions for making EvilUpdater more beautiful *-* (.cpp file)

#include "console.misc.hpp"

bool cmisc::ReloadConsoleScreen(int backgroundcolor, int foregroundcolor)
{
...
bool cmisc::coutc(char msg[257], int backgroundc, int foregroundc, int dbackground, int dforeground)
{
    // Set color
    if(!basic_cmisc::colorchange(backgroundc, foregroundc)) std::cout << "error!";
    ...
    // Set color
    if(!basic_cmisc::colorchange(dbackground, dforeground)) std::cout << "error!";

    // Now, return...
    return true;
}

bool cmisc::basic_cmisc::colorchange(int cbackground, int cforeground)
{
   // Calculates handle
    HANDLE hOut;
